...
/**
 * Set OauthBearer token
 */
Client.prototype.setOauthBearerToken = function(oauthBearerToken) {
  if (!this.isConnected()) {
    return cb(new Error('Client is disconnected'));
  }

  this._client.setOauthBearerToken(oauthBearerToken);
}

/**
 * Set OauthBearer Failure
 *
 * Set the failure will trigger the refresh event in 10s
 */
Client.prototype.setOauthBearerTokenFailure = function(errorString) {
  if (!this.isConnected()) {
    return cb(new Error('Client is disconnected'));
  }

  this._client.setOauthBearerTokenFailure(errorString);
}
/**
